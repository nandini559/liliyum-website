...
export const LuxurySection: React.FC = () => {
  ...
  return (
    <section className="py-16 sm:py-24 bg-[#FAF3EA] text-[#3B1C10] w-full max-w-full overflow-hidden relative select-none border-t border-[#EAE0D5]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full relative z-10">
        ...
        <div className="flex items-center justify-center gap-2 sm:gap-3 overflow-x-auto pb-4 mb-10 no-scrollbar max-w-full">
          {filterTabs.map((tab) => (
            <button
              ...
              className={`px-5 py-2 rounded-full text-xs sm:text-sm font-semibold transition-all duration-300 cursor-pointer whitespace-nowrap ${
                activeFilter === tab
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start mb-12">
              ...
              <div key={card.id} className="flex flex-col group">
                ...
                <div className="flex items-center justify-between px-1">
                  ...
                  <span className="font-serif text-base sm:text-lg font-extrabold text-[#3B1C10]">
                    {card.priceUsd} <span className="text-xs text-[#5C382A]/70 font-normal">({card.priceInr})</span>
                  </span>
                </div>
              </div>
            ))}
          ...
          <div className="lg:col-span-5 flex flex-col group h-full">
            <Link to={`/product/${tallRightCard.id}`} className="block overflow-hidden rounded-2xl mb-3 shadow-md group-hover:shadow-lg transition-all flex-grow">
              <img
                src={tallRightCard.image}
                alt={tallRightCard.name}
                className="w-full h-[460px] sm:h-[540px] lg:h-[570px] object-cover group-hover:scale-105 transition-transform duration-500"
                loading="lazy"
              />
            </Link>
            ...
            <div className="flex items-center justify-between px-1">
              ...
              <span className="font-serif text-lg sm:text-xl font-extrabold text-[#3B1C10]">
                {tallRightCard.priceUsd} <span className="text-xs text-[#5C382A]/70 font-normal">({tallRightCard.priceInr})</span>
              </span>
            </div>
          </div>
        ...
        <div className="text-center">
          <Link
            to="/collection?category=belgian-chocolates"
            className="inline-flex items-center gap-2 bg-[#3B1C10] hover:bg-[#28120A] text-[#FAF3EA] px-8 py-3.5 rounded-full font-serif font-bold text-base shadow-xl transition-all duration-300 transform hover:-translate-y-0.5"
          >
