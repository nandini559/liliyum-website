...
export const BestsellerSection: React.FC = () => {
  ...
  return (<section className="py-16 sm:py-24 bg-amber-200 text-amber-950 w-full max-w-full overflow-hidden select-none">
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
      {/* Main Section Header */}
      <h2 className="font-serif text-4xl sm:text-5xl md:text-6xl font-black text-amber-950 tracking-tight text-center mb-12 sm:mb-16">
          ...
          displayProducts.map((product) => (<div key={product.id} className="flex flex-col group">
            ...
            <div className="flex items-start justify-between gap-3 px-1">
              ...
              <div className="flex flex-col items-end shrink-0 pt-0.5">
                <Link to={`/product/${product.id}`} className="w-11 h-11 rounded-xl bg-amber-900 hover:bg-amber-950 text-white flex items-center justify-center shadow-md transition-all transform hover:scale-105 mb-1.5 cursor-pointer" aria-label={`Order ${product.name}`}>
